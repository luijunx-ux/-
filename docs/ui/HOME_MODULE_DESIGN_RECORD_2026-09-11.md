...
## 9. 五运六气 Flutter 详情页（2026-09-19）

- Vitality 首页“五运六气”入口已接入独立详情页；Forest Life 与 Obsidian Life 暂时保留原详情结构。
- 双列表格仅展示规则引擎当前真实返回的年度干支、中运、司天、在泉和算法版本；右栏使用“说明含义—如何使用—不能据此判断什么”的指导结构，帮助普通用户把术语放回真实日期、天气与个人感受中理解。
- 当前接口未提供客运、当前节气、实时地点、天气或空气质量时，页面明确显示缺失状态，不补造结果。
- 出生地点明确标记为“档案出生地”，不得冒充用户实时所在地；五运六气与环境信息不参与生命参考分加减。
- 四类生活提示为已审核的通用建议，不声明由具体五运六气术语推导，也不构成医疗建议。
- 视觉审核基线：[`review/vitality-yunqi-detail-mobile-v1.png`](review/vitality-yunqi-detail-mobile-v1.png)；用户已于 2026-09-19 明确确认整体视觉界面审核通过。

## 10. 星辰节律 Flutter 详情页（2026-09-19）

- Vitality 首页“星辰节律”入口接入独立详情页，与五运六气保持同等级入口、接近的信息量和相同导航深度；Forest Life 与 Obsidian Life 暂不改动。
- MVP 仅展示规则结果中已经返回的太阳星座、元素与模式；月亮星座和上升星座明确显示为“未来能力 · 当前不推测”，不补造完整星盘。
- 双列表格使用“专业术语与结果 / 普通人怎么理解与使用”结构，明确每项的含义、可用于怎样的自我观察，以及不能据此判断什么。
- 专注、沟通、情绪和休息只作为开放式观察提示，不宣称由星座推导健康事实，不提供桃花、财富、吉凶或具体事件预测。
- 星辰节律仅作文化与自我观察背景，不参与生命参考分加减；星座结果由确定性规则计算，LLM 不参与结果计算。
- 视觉审核基线：[`review/vitality-stellar-detail-mobile-v1.png`](review/vitality-stellar-detail-mobile-v1.png)；用户已于 2026-09-19 明确回复“视觉审核通过”。

## 11. 四类生活建议单项展开（2026-09-19）

- “今天怎样照顾自己”保留饮食、睡眠、活动、情绪四张首页摘要卡；点击后在原板块内展开，任一时刻只显示一个分类的具体内容。
- AI 综合分析仍是唯一主任务，四类建议统一标记为补充建议，板块顶部提供返回 AI 今日优先行动的入口，避免形成多个并列主任务。
- 每个分类包含一个温和行动、两步做法、依据与数据缺失说明；当前没有饮食、睡眠、活动、Readiness 等数据时只使用已审核的通用建议，不伪造个性化判断。
- 完成、跳过、调整和替换为当前页面的本地交互演示，不上传服务端、不写入健康事实，也不影响已经生成的生命参考分；正式持久化须等待分类建议服务端契约冻结。
- 视觉审核基线：[`review/vitality-daily-advice-expanded-v1.png`](review/vitality-daily-advice-expanded-v1.png)；用户已于 2026-09-20 明确回复“视觉审核通过”。

## 12. 我的重点状态与今日记录深化（2026-09-22）

- 首页重点状态板块补齐精力、压力感受和身体感受三组单选记录，以及不超过 120 字的可选备注；所有选项触控区域不小于 44×44。
- 睡眠与活动继续作为只读客观数据展示；健康平台未授权时明确显示“暂无数据”和来源状态，不使用概念数值。
- 用户选择“明显不适”时仅显示温和的专业帮助提示，不修改分值、不生成诊断或治疗建议。
- 保存后显示“今日已记录 1 次”，时间线追加用户主动记录；用户可继续编辑当前内容或删除记录。当前交互仅保存在页面内，尚未接入服务端持久化。
- 系统事件与用户主观记录使用不同来源说明；本次记录不修改已经生成的生命参考分，也不会在未经授权和冻结规则的情况下参与后续计算。
- 视觉审核基线：[`review/vitality-focus-checkin-expanded-v1.png`](review/vitality-focus-checkin-expanded-v1.png)；用户已于 2026-09-22 明确回复“视觉审核通过”。

## 13. Vitality 底部导航校正（2026-09-22）

- 修正“记录”入口原先错误打开“每日建议历史”的语义偏差；在主观记录尚未接入持久化前，不伪造独立记录中心。
- “今日”平滑返回每日生活主图，“节律”定位五运六气与星辰节律双核心入口，“记录”定位“我的重点状态与今日记录”，“我的”继续进入账户页。
- 导航仅改变页面定位行为，不改变已通过审核的底部导航视觉、首页信息结构或 Forest Life / Obsidian Life 页面。
- 每个导航触控区域继续保持至少 44×44；新增 Widget 测试验证三个页内入口均能把真实目标板块移入可视区域。
